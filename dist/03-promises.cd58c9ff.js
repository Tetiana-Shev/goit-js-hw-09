...
function onFormSubmit(e) {
    e.preventDefault();
    let delay1 = Number(document.querySelector('[name="delay"]').value);
    let step = Number(document.querySelector('[name="step"]').value);
    let amount = Number(document.querySelector('[name="amount"]').value);
    if (delay1 >= 0 && step >= 0) for(let position1 = 1; position1 <= amount; position1 += 1){
        createPromise(position1, delay1).then(({ position , delay  })=>(0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${position} in ${delay}ms`)).catch(({ position , delay  })=>(0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${position} in ${delay}ms`));
        delay1 += step;
    }
    else {
